...
function themeLabel(){const dark=document.documentElement.dataset.theme==='dark';if(themeButton){themeButton.textContent=dark?'المظهر الفاتح':'المظهر الداكن';themeButton.setAttribute('aria-pressed',String(dark));}}
themeLabel();themeButton?.addEventListener('click',()=>{const theme=document.documentElement.dataset.theme==='dark'?'light':'dark';document.documentElement.dataset.theme=theme;try{localStorage.setItem('theme',theme)}catch(e){}themeLabel();});
